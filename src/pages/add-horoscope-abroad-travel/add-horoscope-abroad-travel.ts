...
import { AddHoroscopeLastCallPage } from '../add-horoscope-last-call/add-horoscope-last-call';

/**
 * Generated class for the AddHoroscopeAbroadTravelPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-add-horoscope-abroad-travel',
  templateUrl: 'add-horoscope-abroad-travel.html',
})
export class AddHoroscopeAbroadTravelPage {
  /**
  ...
  constructor(public navCtrl: NavController,
    ...
    this.alertProvider.setCurrentPage(AddHoroscopeAbroadTravelPage);
  ...
  */
  ...
  ionViewDidLoad() {
    //console.log('ionViewDidLoad AddHoroscopeAbroadTravelPage');
  }
  ...
  setAddHoroscopeDataIfExist() {
    if (this.horoscopeData) {
      if (this.horoscopeData.travelDay) {
        this.travelCalendar.setSelectedDate = this.horoscopeData.travelDay;
      }
      if (this.horoscopeData.travelMonth) {
        this.travelCalendar.setSelectedMonth = this.horoscopeData.travelMonth;
      }
      if (this.horoscopeData.travelYear) {
        this.travelCalendar.setSelectedYear = this.horoscopeData.travelYear;
      }
      if (this.horoscopeData.travelHour) {
        this.travelTime.setSelectedHour = this.horoscopeData.travelHour;
      }

      if (this.horoscopeData.travelMin) {
        this.travelTime.setSelectedMin = this.horoscopeData.travelMin;
      }

      if (this.horoscopeData.travelSec) {
        this.travelTime.setSelectedSec = this.horoscopeData.travelSec;
      }

      if (this.horoscopeData.travelAmPm) {
        this.travelTime.setSelectedAmPm = this.horoscopeData.travelAmPm;
      }
    }
  }
  ...
  setHoroscopeDataForUpdate() {
    if (this.horoscopeData
      && this.horoscopeData.getHAtDate
      && this.horoscopeData.getHAtDate != Constants.BLANK_DATE_TIME) {
      if (this.horoscopeData.getHAtDate.includes("-") || this.horoscopeData.getHAtDate.includes("/")) {
        ...
        this.horoscopeData.travelDay = this.travelCalendar.getSelectedDate;
        this.horoscopeData.travelMonth = this.travelCalendar.getSelectedMonth;
        this.horoscopeData.travelYear = this.travelCalendar.getSelectedYear;
      }
    }


    if (this.horoscopeData
      && this.horoscopeData.getHAtTime
      && this.horoscopeData.getHAtTime != Constants.BLANK_DATE_TIME) {
      if (this.horoscopeData.getHAtTime.includes("-") || this.horoscopeData.getHAtTime.includes("/")) {
        ...
        this.horoscopeData.travelHour = this.travelTime.getSelectedHour;
        
        this.horoscopeData.travelMin = this.travelTime.getSelectedMin;
        
        this.horoscopeData.travelSec = this.travelTime.getSelectedSec
        
        this.horoscopeData.travelAmPm = this.travelTime.getSelectedAmPm;
  ...
  getImage() {
    ...
    this.camera.getPicture(options).then((imageData) => {
      ...
      //console.log(err);
      //alert(err);
      this.alertProvider.basicAlertOnPage(AddHoroscopeAbroadTravelPage,null, err, null);
  ...
  selectTravelCalendar(type: number) {
    ...
    popover.onDidDismiss(popOverData => {
      ...
      if (type == Constants.CALENDAR_DATE) {
        //for local
        this.horoscopeData.travelDay = popOverData.item;

        this.travelCalendar.setSelectedDate = popOverData.item;
      } else if (type == Constants.CALENDAR_MONTH) {
        //for local
        this.horoscopeData.travelMonth = this.travelCalendar.getMonthNumberFromName(popOverData.item);
        ...
        if (this.travelCalendar.selectedYear
          ...
          if (this.travelCalendar.getSelectedYear
            ...
            if (Number(this.travelCalendar.getSelectedDate) > 29) {
              //for local
              this.horoscopeData.travelDay = null;
        ...
        //for local
        this.horoscopeData.travelYear = popOverData.item;
        ...
        if (this.travelCalendar.getSelectedMonth) {
          ...
          if (Number(this.travelCalendar.getSelectedYear) == this.travelCalendar.currentYear) {
            if (Number(this.travelCalendar.getSelectedMonth) > this.travelCalendar.currentMonth) {
              //for local
              this.horoscopeData.travelMonth = null;
  ...
  selectTravelTime(timeType: number) {
    ...
    popover.onDidDismiss(popOverData => {
      ...
      if (timeType == Constants.TIME_HOUR) {
        //for local
        this.horoscopeData.travelHour = popOverData.item;

        this.travelTime.setSelectedHour = popOverData.item;

      } else if (timeType == Constants.TIME_MIN) {
        //for local
        this.horoscopeData.travelMin = popOverData.item;

        this.travelTime.setSelectedMin = popOverData.item;

      } else if (timeType == Constants.TIME_SEC) {
        //for local
        this.horoscopeData.travelSec = popOverData.item;

        this.travelTime.setSelectedSec = popOverData.item;

      } else if (timeType == Constants.TIME_AM_PM) {
        //for local
        this.horoscopeData.travelAmPm = popOverData.item;
  ...
  saveProfile(status: string) {
    ...
    try {
      this.horoscopeData.setHAtDate = this.getEventDate(this.travelCalendar);
    } catch (e) {

    }

    try {
      this.horoscopeData.setHAtTime = this.getEventTime(this.travelTime);
    } catch (e) {

    }
    this.horoscopeData.setHAtPlace = (this.horoscopeData.getHAtPlace && this.horoscopeData.getHAtPlace != null && this.horoscopeData.getHAtPlace != 'null') ? this.horoscopeData.getHAtPlace : "";
    this.horoscopeData.setHAtTAmpm = (this.travelTime && this.travelTime.getSelectedAmPm) ? this.travelTime.getSelectedAmPm : null;
    this.horoscopeData.setHAFlightNo = this.horoscopeData.getHAFlightNo;

    this.navCtrl.push(AddHoroscopeLastCallPage, {
      type: this.type,
  ...
  validate(): boolean {
    ...
    if (this.travelCalendar && (this.travelCalendar.getSelectedDate
      || this.travelCalendar.getSelectedMonth
      || this.travelCalendar.getSelectedYear)) {
      if (!this.travelCalendar.getSelectedDate || this.travelCalendar.getSelectedDate === "00") {
        //alert(strings.please_select_marriage_date);
        this.isValidTravelDate = false;
        this.alertProvider.basicAlertOnPage(AddHoroscopeAbroadTravelPage,null, strings.please_select_travel_date, null);
        return false;
      } if (!this.travelCalendar.getSelectedMonth || this.travelCalendar.getSelectedMonth === "00") {
        //alert(strings.please_select_marriage_month);
        this.isValidTravelMonth = false;
        this.alertProvider.basicAlertOnPage(AddHoroscopeAbroadTravelPage,null, strings.please_select_travel_month, null);
        return false;
      } if (!this.travelCalendar.getSelectedYear || this.travelCalendar.getSelectedYear === "00") {
        //alert(strings.please_select_marriage_year);
        this.isValidTravelYear = false;
        this.alertProvider.basicAlertOnPage(AddHoroscopeAbroadTravelPage,null, strings.please_select_travel_year, null);
        return false;
      }
    }
    ...
    if (this.travelTime && (this.travelTime.getSelectedHour
      ...
      if (!this.travelTime.getSelectedHour || this.travelTime.getSelectedHour === "00") {
        //alert(strings.please_select_marriage_date);
        this.isValidTravelHours = false;
        this.alertProvider.basicAlertOnPage(AddHoroscopeAbroadTravelPage,null, strings.please_select_travel_hour, null);
        return false;
      }
      if (!this.travelTime.getSelectedMin) {
        //alert(strings.please_select_marriage_month);
        this.isValidTravelMin = false;
        this.alertProvider.basicAlertOnPage(AddHoroscopeAbroadTravelPage,null, strings.please_select_travel_min, null);
        return false;
      }
      // if (!this.travelTime.getSelectedSec) {
      //   //alert(strings.please_select_marriage_year);
      //   this.isValidTravelSec = false;
      //   this.alertProvider.basicAlertOnPage(AddHoroscopeAbroadTravelPage,null, strings.please_select_travel_sec, null);
      //   return false;
      // }
      if (!this.travelTime.getSelectedAmPm) {
        //alert(strings.please_select_marriage_year);
        this.alertProvider.basicAlertOnPage(AddHoroscopeAbroadTravelPage,null, strings.please_select_travel_am_pm, null);
        return false;
      }
    }
    return true;
      ...
      .then((response) => {
        ...
        if (data && data.getStatus === 'Success') {
          ...
          if (data.getData) {
            ...
            this.alertProvider.basicAlertOnPage(AddHoroscopeAbroadTravelPage,null, data.getMessage, null);
        ...
        this.alertProvider.basicAlertOnPage(AddHoroscopeAbroadTravelPage,null, JSON.stringify(err), null);
      });
  }
  ...
  updateHoroscopeOnlyData(req: any) {
    ...
    this.api.updateHoroscope(postData).subscribe(response => {
      ...
      // this.alertProvider.basicAlertOnPage(AddHoroscopeAbroadTravelPage,null, strings.server_error, null);

      if(error.toString().includes(Constants.RESPONSE_WITH_STATUS_0_1) 
        || error.toString().includes(Constants.RESPONSE_WITH_STATUS_0_2)){
        this.alertProvider.basicAlertOnPage(AddHoroscopeAbroadTravelPage,null, strings.please_check_internet_access, null);
      }else{
        this.alertProvider.basicAlertOnPage(AddHoroscopeAbroadTravelPage,null,error.toString(), null);
      }
