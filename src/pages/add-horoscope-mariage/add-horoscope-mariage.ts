...
import { AddHoroscopeChildPage } from '../add-horoscope-child/add-horoscope-child';
/**
 * Generated class for the AddHoroscopeMariagePage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-add-horoscope-mariage',
  templateUrl: 'add-horoscope-mariage.html',
})
export class AddHoroscopeMariagePage {
  ...
  constructor(public navCtrl: NavController,
    ...
    public translate: TranslateService) {
      ...
      this.alertProvider.setCurrentPage(AddHoroscopeMariagePage);
  ...
  */
  ...
  ionViewDidLoad() {
    //console.log('ionViewDidLoad AddHoroscopeMarriagePage');
  }
  ...
  setAddHoroscopeDataIfExist() {
    if (this.horoscopeData) {
      if (this.horoscopeData.marriageDay) {
        this.marriageCalendar.setSelectedDate = this.horoscopeData.marriageDay;
      }
      if (this.horoscopeData.marriageMonth) {
        this.marriageCalendar.setSelectedMonth = this.horoscopeData.marriageMonth;
      }
      if (this.horoscopeData.marriageYear) {
        this.marriageCalendar.setSelectedYear = this.horoscopeData.marriageYear;
      }

      if (this.horoscopeData.marriageHour) {
        this.marriageTime.setSelectedHour = this.horoscopeData.marriageHour;
      }

      if (this.horoscopeData.marriageMin) {
        this.marriageTime.setSelectedMin = this.horoscopeData.marriageMin;
      }

      if (this.horoscopeData.marriageSec) {
        this.marriageTime.setSelectedSec = this.horoscopeData.marriageSec;
      }

      if (this.horoscopeData.marriageAmPm) {
        this.marriageTime.setSelectedAmPm = this.horoscopeData.marriageAmPm;
      }
    }
  }
  ...
  setHoroscopeDataForUpdate() {
    ...
    if (this.horoscopeData.getHMarriageDate 
      && this.horoscopeData.getHMarriageDate != Constants.BLANK_DATE_TIME) {
        if(this.horoscopeData.getHMarriageDate.includes("-") || this.horoscopeData.getHMarriageDate.includes("/")){
          ...
          this.horoscopeData.marriageDay = this.marriageCalendar.getSelectedDate;
          this.horoscopeData.marriageMonth = this.marriageCalendar.getSelectedMonth;
          this.horoscopeData.marriageYear = this.marriageCalendar.getSelectedYear;
        }
    }

    if (this.horoscopeData.getHMarriageTime  
      && this.horoscopeData.getHMarriageTime != Constants.BLANK_DATE_TIME) {
        if(this.horoscopeData.getHMarriageTime.includes("-") || this.horoscopeData.getHMarriageTime.includes("/")){
          ...
          this.marriageTime.setSelectedSec = this.getTimeFromComplete(this.horoscopeData.getHMarriageTime.toString(), Constants.TIME_SEC);
            ...
            this.horoscopeData.marriageHour = this.marriageTime.getSelectedHour;
            this.horoscopeData.marriageMin=this.marriageTime.getSelectedMin;
            this.horoscopeData.marriageSec = this.marriageTime.getSelectedSec 
            this.horoscopeData.marriageAmPm = this.marriageTime.getSelectedAmPm;
          
        }
    }

    if (this.horoscopeData.getHMarriageAmpm) {
  ...
  getImage() {
    ...
    this.camera.getPicture(options).then((imageData) => {
      ...
      //console.log(err);
      //alert(err);
      this.alertProvider.basicAlertOnPage(AddHoroscopeMariagePage,null, err, null);
  ...
  selectMarriageCalendar(type: number) {
    ...
    popover.onDidDismiss(popOverData => {
      ...
      if (type == Constants.CALENDAR_DATE) {
        //for local
        this.horoscopeData.marriageDay = popOverData.item;

        this.marriageCalendar.setSelectedDate = popOverData.item;
      } else if (type == Constants.CALENDAR_MONTH) {
        //for local
        this.horoscopeData.marriageMonth = this.marriageCalendar.getMonthNumberFromName(popOverData.item);
        //get month nummber from name
        ...
        if (this.marriageCalendar.selectedYear
          ...
          //for local
          this.horoscopeData.marriageDay = null;
        ...
        //for local
        this.horoscopeData.marriageYear = popOverData.item;
        ...
        if (this.marriageCalendar.getSelectedMonth) {
          ...
          if (Number(this.marriageCalendar.getSelectedYear) == this.marriageCalendar.currentYear) {
            if (Number(this.marriageCalendar.getSelectedMonth) > this.marriageCalendar.currentMonth) {
              //for local
              this.horoscopeData.marriageMonth = null;
  ...
  selectMarriageTime(timeType: number) {
    ...
    popover.onDidDismiss(popOverData => {
      ...
      if (timeType == Constants.TIME_HOUR) {
        //for local
        this.horoscopeData.marriageHour = popOverData.item;

        this.marriageTime.setSelectedHour = popOverData.item;
      } else if (timeType == Constants.TIME_MIN) {
        //for local
        this.horoscopeData.marriageMin = popOverData.item;
        ...
        //for local
        this.horoscopeData.marriageSec = popOverData.item;

        this.marriageTime.setSelectedSec = popOverData.item;

      } else if (timeType == Constants.TIME_AM_PM) {
        //for local
        this.horoscopeData.marriageAmPm = popOverData.item;
  ...
  saveProfile(status: string) {
    ...
    try{
      this.horoscopeData.setHMarriageDate = this.getEventDate(this.marriageCalendar);
    }catch(e){

    }

    try{
      this.horoscopeData.setHMarriageTime = this.getEventTime(this.marriageTime);
    }catch(e){

    }
    
    
    this.horoscopeData.setHMarriageAmpm = (this.marriageTime && this.marriageTime.getSelectedAmPm) ? this.marriageTime.getSelectedAmPm : null;
    this.horoscopeData.setHMarriagePlace = (this.horoscopeData.getHMarriagePlace && this.horoscopeData.getHMarriagePlace != null && this.horoscopeData.getHMarriagePlace != 'null') ? this.horoscopeData.getHMarriagePlace : "";

    this.navCtrl.push(AddHoroscopeChildPage, {
      type: this.type,
  ...
  validate(): boolean {
    ...
    // if(this.horoscopeData.marriageDay || this.horoscopeData.marriageMonth || this.horoscopeData.marriageYear){
    //   if(!this.horoscopeData.marriageDay){
    //     this.alertProvider.basicAlertOnPage(AddHoroscopeMariagePage,null, strings.please_select_marriage_date, null);
    //     return false;
    //   }
    //   else if(!this.horoscopeData.marriageMonth){
    //     this.alertProvider.basicAlertOnPage(AddHoroscopeMariagePage,null, strings.please_select_marriage_month, null);
    //     return false;
    //   }
    //   else if(!this.horoscopeData.marriageYear){
    //     this.alertProvider.basicAlertOnPage(AddHoroscopeMariagePage,null, strings.please_select_marriage_year, null);
    //     return false;
    //   }
    // }

    // if(this.horoscopeData.marriageHour || this.horoscopeData.marriageMin || this.horoscopeData.marriageSec){
    //   if(!this.horoscopeData.marriageHour){
    //     this.alertProvider.basicAlertOnPage(AddHoroscopeMariagePage,null, strings.please_select_marriage_hour, null);
    //     return false;
    //   }
    //   else if(!this.horoscopeData.marriageMin){
    //     this.alertProvider.basicAlertOnPage(AddHoroscopeMariagePage,null, strings.please_select_marriage_min, null);
    //     return false;
    //   }
    //   else if(!this.horoscopeData.marriageSec){
    //     this.alertProvider.basicAlertOnPage(AddHoroscopeMariagePage,null, strings.please_select_marriage_sec, null);
    //     return false;
    //   }
    // }

    if(this.marriageCalendar && (this.marriageCalendar.getSelectedDate 
      || this.marriageCalendar.getSelectedMonth 
      || this.marriageCalendar.getSelectedYear)){
        if(!this.marriageCalendar.getSelectedDate || this.marriageCalendar.getSelectedDate === "00"){
          //alert(strings.please_select_marriage_date);
          this.isValidMarriageDate = false;
          this.alertProvider.basicAlertOnPage(AddHoroscopeMariagePage,null, strings.please_select_marriage_date, null);
          return false;
        }if(!this.marriageCalendar.getSelectedMonth || this.marriageCalendar.getSelectedMonth === "00"){
          //alert(strings.please_select_marriage_month);
          this.isValidMarriageMonth = false;
          this.alertProvider.basicAlertOnPage(AddHoroscopeMariagePage,null, strings.please_select_marriage_month, null);
          return false;
        }if(!this.marriageCalendar.getSelectedYear || this.marriageCalendar.getSelectedYear === "00"){
    ...
    if(this.marriageTime && (this.marriageTime.getSelectedHour 
      ...
      || this.marriageTime.getSelectedAmPm)){
        if(!this.marriageTime.getSelectedHour || this.marriageTime.getSelectedHour === "00"){
          //alert(strings.please_select_marriage_date);
        ...
        if(!this.marriageTime.getSelectedAmPm){
          //alert(strings.please_select_marriage_year);
          this.alertProvider.basicAlertOnPage(AddHoroscopeMariagePage,null, strings.please_select_marriage_am_pm, null);
          return false;
        }
      }
      ...
      .then((response) => {
        ...
        if (data && data.getStatus === 'Success') {
          ...
          if (data.getData) {
            ...
            this.alertProvider.basicAlertOnPage(AddHoroscopeMariagePage,null, data.getMessage, null);
        ...
        this.alertProvider.basicAlertOnPage(AddHoroscopeMariagePage,null, JSON.stringify(err), null);
      });
  }
  ...
  updateHoroscopeOnlyData(req: any) {
    ...
    this.api.updateHoroscope(postData).subscribe(response => {
      ...
      // this.alertProvider.basicAlertOnPage(AddHoroscopeMariagePage,null, strings.server_error, null);

      if(error.toString().includes(Constants.RESPONSE_WITH_STATUS_0_1) 
        || error.toString().includes(Constants.RESPONSE_WITH_STATUS_0_2)){
        this.alertProvider.basicAlertOnPage(AddHoroscopeMariagePage,null, strings.please_check_internet_access, null);
      }else{
        this.alertProvider.basicAlertOnPage(AddHoroscopeMariagePage,null,error.toString(), null);
      }
    }, () => {
      //this.loader.dismiss();
